 ...
 *
 * Usage:
 *   ./bitk-launcher              # run from binary location
 *   BITK_ROOT=/opt/bitk ./bitk   # override root directory
 */
...
const ROOT_DIR = process.env.BITK_ROOT
  ? resolve(process.env.BITK_ROOT)
...
const GITHUB_REPO = 'bkhq/bitk'
const APP_PKG_RE = /^bitk-app-v(\d+\.\d+\.\d+)\.tar\.gz$/
...
async function downloadToFile(
  ...
  try {
    const res = await fetch(url, {
      headers: { 'User-Agent': 'bitk-launcher' },
...
async function main() {
  ...
  if (!version) {
    ...
    if (!latest) {
      ...
      console.error(
        `  3. tar -xzf bitk-app-v<VERSION>.tar.gz -C ${APP_BASE}/v<VERSION>`,
      )
