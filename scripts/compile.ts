 ...
 *   bun scripts/compile.ts --mode launcher --outfile bitk-launcher
 *   bun scripts/compile.ts --mode launcher --target bun-linux-x64 --outfile bitk-launcher-linux-x64
 */
import {
  copyFileSync,
  existsSync,
...
if (compileMode === 'launcher') {
  ...
  const defaultOutfile =
    version !== 'dev' ? `bitk-launcher-${version}` : 'bitk-launcher'
  ...
  const binaryName = outfile.split('/').pop() ?? 'bitk-launcher'
...
`
  ...
  const defaultOutfile = version !== 'dev' ? `bitk-${version}` : 'bitk'
  mkdirSync(OUT_DIR, { recursive: true })
  const outfile = resolve(
    OUT_DIR,
    (args.outfile as string | undefined) ?? defaultOutfile,
  ...
  const sha256 = hasher.digest('hex')
  const checksumFile = resolve(OUT_DIR, 'checksums.txt')
  const binaryName = outfile.split('/').pop() ?? 'bitk'
  const entry = `${sha256}  ${binaryName}\n`
  const existing = existsSync(checksumFile)
    ? await Bun.file(checksumFile).text()
    : ''
